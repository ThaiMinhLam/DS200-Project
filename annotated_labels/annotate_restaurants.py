...
def extract_price(text):
    price_keywords = [
        r"\b(giá|phí)\s*(ăn|món|đồ ăn|mì|bún|phở|nem|bánh|tráng miệng|chè|nước|trà|bia|ly|chai|menu|quán|dĩa|suất|combo|set|ốc|beefsteak|bbq|nướng|lẩu)\b",
...
# Gán nhãn FOOD#PRESENTATION
def extract_food_presentation(text):
    food_keywords = [
        r"\b(phần|đồ|món|thức) ăn\b", r"\bkhẩu phần\b", r"\bphần ăn\b", r"\bcơm\b", r"\blẩu\b", r"\bhải sản\b",
        r"\bgỏi\b", r"\bcanh\b", r"\bphở\b", r"\bbún\b", r"\bthịt\b", r"\bmì\b", "pizza", "beefsteak", r"\bsalad\b"
    ]
    presentation_keywords = [
        r"\btrình bày\b", r"\btrang trí\b", r"\bdecor\b", r"\bbày biện\b", 
        r"\btrang hoàng\b", r"\bdecor món\b", r"\bbày trí\b", r"\bsắp xếp\b",
        r"\bthể hiện\b", r"\btạo hình\b", r"\bcách bày\b", r"\bcách thể hiện\b",
        r"\bpresentation\b", r"\barrangement\b", r"\bplating\b"
    ]

    existAspect = any(re.search(normalize_unicode(food_keyword), text, re.IGNORECASE) for food_keyword in food_keywords) and any(re.search(normalize_unicode(presentation_keyword), text, re.IGNORECASE) for presentation_keyword in presentation_keywords)
    if existAspect:
        positive_patterns = [
            r"(trình bày|trang trí|decor|bày biện|bày trí).{0,10}?(rất|cực|quá|vô cùng|thật|hết sức|đặc biệt|khá|vừa)?\s*(đẹp|bắt mắt|tinh tế|gọn gàng|sạch sẽ|ấn tượng|xinh xắn|chuyên nghiệp|sáng tạo|độc đáo|nghệ thuật|hoàn hảo|chỉn chu)",
            r"món ăn.{0,10}?(trình bày|bày biện).{0,10}?(đẹp|ấn tượng|hấp dẫn|tuyệt vời|chuẩn)",
            r"(đẹp|ấn tượng|bắt mắt|tuyệt vời).{0,10}?(trình bày|decor|trang trí)",
            r"(?<!\bchưa\s)(trình bày|bày biện).{0,15}?(đẹp hơn|ấn tượng hơn|chuyên nghiệp hơn).{0,10}?(so với|hơn|vượt)",
            r"(đĩa|dĩa)\s*(đồ|món|thức)\s*ăn.{0,5}?(đẹp|ấn tượng|hấp dẫn)"
        ]
        negative_patterns = [
            r"(trình bày|trang trí|decor|bày biện|bày trí).{0,10}?(quá|rất|cực|vô cùng|thật|hết sức|đặc biệt|khá)?\s*(xấu|cẩu thả|sơ sài|bừa bộn|rối mắt|luộm thuộm|đơn điệu|nhàm chán|tệ|kém|thô lỗ)",
            r"món ăn.{0,10}?(trình bày|bày biện).{0,10}?(xấu|kém|tệ|không đẹp|không ổn)",
            r"(xấu|tệ|kém|nhạt nhẽo).{0,10}?(trình bày|decor|trang trí)",
            r"(không|chẳng|đâu có).{0,5}?(đẹp|bắt mắt|tinh tế|gọn gàng).{0,10}?(trình bày|trang trí)"
        ]
...
# Gán nhãn FOOD#FRESHNESS
def extract_food_freshness(text):
    freshness_keywords = [
        r"\btươi\s*(sống|mới|roi rói)?\b",
        r"\bkhông tươi\b", r"\bươn\b", r"\b(bị )?hôi\b", r"\bcó mùi\b", r"\bmùi lạ\b",
        r"\bthiu\b", r"\bôi\b", r"\bbị ôi\b", r"\bthiếu tươi\b",
        r"\bfresh\b", r"\bfreshness\b", r"\bstale\b", r"\bspoiled\b", r"\boff smell\b", r"\brotten\b",
        r"\bcòn sống\b", r"\bmới thu hoạch\b", r"\bmới đánh bắt\b", r"\bchế biến tươi\b", r"\bnguyên liệu tươi\b"
    ]
    
    existAspect = any(re.search(normalize_unicode(keyword), text, re.IGNORECASE) for keyword in freshness_keywords)
    if existAspect:
        positive_patterns = [
            r"\b(tươi sống|rất tươi|tươi mới|tươi roi rói|tươi ngon|siêu tươi|cực tươi)\b",
            r"\b(còn sống|mới thu hoạch|mới đánh bắt|chế biến tươi|nguyên liệu tươi)\b",
            r"\b(fresh|very fresh|extremely fresh)\b",
            r"(hải sản|đồ ăn|thức ăn|món).{0,10}?(tươi|fresh)",
            r"\bđộ tươi.*?(tốt|hoàn hảo|tuyệt vời|đáng kinh ngạc)\b",
            r"\b(tươi).{0,5}?(ngon|hấp dẫn|chất lượng|tốt)\b",
            r"\b(đảm bảo|giữ được|giữ nguyên).{0,8}?độ tươi\b"
        ]
        negative_patterns = [
            r"\b(không tươi|ươn|bị hôi|có mùi|mùi lạ|mùi khó chịu|ôi|thiu|thiếu tươi|kém tươi)\b",
            r"\b(bị ôi|đã để lâu|cũ|đông lạnh lâu|đã rã đông|không còn tươi)\b",
            r"\b(stale|spoiled|rotten|off smell|not fresh|unfresh)\b",
            r"\b(tươi).{0,5}?(giả|không đảm bảo|kém)\b",
            r"\b(mùi).{0,5}?(hôi|ôi|khó chịu|lạ)\b",
            r"\b(độ tươi).{0,5}?(kém|thấp|không tốt|không ổn)\b",
            r"\b(cảm giác|dường như|hình như).{0,10}?không tươi"
        ]
...
def extract_service_staff(text):
    ...
    staff_activities = [
        r"\b(cục súc|hòa đồng|cộc cằn|thái độ|phục vụ|nhiệt tình|thuyết minh|giới thiệu|hướng dẫn|tư vấn|giải đáp|đón tiếp|tiếp đón|quan tâm|dịch vụ)\b",
...
aspect_detection_functions = {
    ...
    "FOOD#PRESENTATION": extract_food_presentation,
    "FOOD#FRESHNESS": extract_food_freshness,
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./DatasetGGMaps/final_review_googlemap_comments_restaurant.csv", index_col=0)
    df1 = pd.read_csv("./DatasetGGMaps/final_review_tripadvisor_comments_restaurant.csv", index_col=0)
    df_new = pd.concat([df, df1], axis=0)
    df_new = append_aspects(df_new, processor)
    df_new.dropna(subset=['text'], ignore_index=True, inplace=True)
    df_new.to_csv('./annotated_labels/list_aspects_restaurants.csv')