...
def extract_price(text):
    price_keywords = [
        r"\b(giá|phí)\s*(cả|đồ uống|nước|cocktail|ly|chai|menu|thức uống|quán|bar|coffee|bia|rượu|trà|cafe|cà phê|sinh tố|nước ép|drink|beverage)\b",
        r"\b(chi phí|hóa đơn|tính tiền|mức giá|giá tiền|bill|price|chi phí|tốn|cắt cổ|treo đầu dê bán thịt chó|bỏ ra|số tiền)\b",
        ...
        r"\bgiá.{0,10}?(đắt|rẻ|hời|bèo|mềm|chát|hợp lý|ổn|sinh viên|văn phòng|hợp túi tiền|vừa túi tiền|premium|cao cấp|bình thường|sốc)\b", 
    ...
    if existAspect:
        positive_patterns = [
            r"\b(giá|chi phí|hóa đơn|bill).{0,10}?(rẻ|hạt dẻ|bình dân|hợp lý|ổn áp|tiết kiệm|hời|bèo|sinh viên|phải chăng|tốt|hấp dẫn|mềm|acceptable|reasonable|affordable)\b",
            r"\b(giá sinh viên|giá văn phòng|hạt dẻ|bèo|xứng đáng|đáng đồng tiền|vừa túi tiền|hợp túi tiền|đáng giá|rất đáng|giảm giá|ưu đãi|phù hợp túi tiền|chất lượng tương xứng|đáng đồng tiền)\b",
            r"\b(ly|chai|combo|set).{0,10}?(giá rẻ|giá tốt|hợp lý|phải chăng)\b",
            r"\b(happy hour|khuyến mãi|giảm giá|promotion).{0,10}?(đáng giá|tốt|hời|worth it)\b",
            r"\b(giá).{0,10}?(ok|ổn áp|hợp lý|fair)\b",
            r"\b(rẻ hơn|tốt hơn|hợp lý hơn).{0,10}?(mong đợi|dự kiến|so với|đa số|chỗ khác)\b",
            r"\b(giá cả).{0,10}?(phải chăng nhất|tốt nhất|rẻ nhất|competitive)\b",
            r"\b(giá cả không thể tốt hơn|đồ uống ngon mà giá cả hợp lý|chất lượng xứng giá|giá cả đi đôi với chất lượng)\b",
        ]
        negative_patterns = [
            r"\b(giá|phí|chi phí|hóa đơn|bill).{0,10}?(bất hợp lý|khủng khiếp|cao|đắt|mắc|cắt cổ|không hợp lý|không đáng|không xứng|chát|sốc|expensive|overprice|trên trời|vô lý|ảo)\b",
            r"\b(ly|chai|combo|set).{0,10}?(giá cao|đắt đỏ|mắc quá|overprice)\b",
            r"\b(giá).{0,15}?(trên trời|cắt cổ|quá cao|khủng khiếp|premium|cao cấp|not worth|cao|chát)\b",
            r"\b(không đáng|tốn tiền|không đáng tiền|không xứng|không hợp lý|too expensive|not worth it|bị chặt chém|bị hố)\b",
            r"\b(đắt hơn|cao hơn|mắc hơn).{0,10}?(mong đợi|dự kiến|so với|đa số|chỗ khác)\b",
            r"\bmất tiền.{0,5}mà không.{0,5}xứng đáng\b",
            r"\b(giá cả).{0,10}?(đắt đỏ nhất|cao nhất|mắc nhất|exorbitant)\b",
...
def extract_food_quality(text):
    food_keywords = [
        r"\b(đồ ăn|món ăn|thức ăn|snack|đồ nhắm|đồ nhâm nhi|đồ ăn kèm|đồ ăn vặt)\b",
        r"\b(bánh ngọt|bánh mì|pizza|burger|pasta|salad|sandwich|tapas|finger food|món phụ)\b",
        r"\b(khoai tây chiên|nuggets|snack mix|đậu phộng|hạt điều|hạt dẻ)\b",
        r"\b(phô mai|cheese|thịt nguội|jamon|salami|cold cut|antipasti)\b",
        r"\b(trái cây|fruit|platter|hải sản|seafood|sushi|sashimi)\b",
        r"\b(nguyên liệu|chất lượng)\s*(tươi|mới)?\b",
        r"\b(menu đồ ăn|thực đơn ăn nhẹ|food menu|appetizer)\b"
    ]

    existAspect = any(re.search(normalize_unicode(keyword), text, re.IGNORECASE) for keyword in food_keywords)
    if existAspect:
        positive_patterns = [
            r"\b(ngon|tuyệt vời|xuất sắc|ấn tượng|đậm đà|thơm ngon|vừa miệng|hợp khẩu vị|tròn vị|tươi ngon|ngon miệng|hài lòng|rất ưng|chuẩn vị)\b",
            r"\b(chất lượng|tuyệt hảo|đỉnh cao|hoàn hảo|perfect|delicious|tasty|excellent)\b",
            r"\b(giòn rụm|thơm nức|mềm ngon|tươi mới|nóng hổi|vừa miệng|cân bằng)\b",
            r"\b(nguyên liệu tươi|homemade|tự làm|handmade|artisanal|hảo hạng)\b",
            r"\b(ngon hơn|tốt hơn|chất lượng hơn).{0,10}?(mong đợi|dự kiến|so với|chỗ khác)\b",
            r"\b(món).{0,10}?(đáng thử|recommend|gợi ý|nên order|best seller)\b",
            r"\b(ăn là ghiền|nghiện món này|nhắm với bia|nhăm nhi với bia|nhắm với rượu|nhắm với rượu|perfect pairing)\b",
            r"\b(snack ngon nhất từng ăn|đồ ăn kèm tuyệt vời|finger food hoàn hảo)\b"
        ]
        negative_patterns = [
            r"\b(dở|tệ|không ngon|nhạt|mặn|chua|đắng|khét|khó ăn|kinh khủng|thất vọng|tồi tệ)\b",
            r"\b(kém chất lượng|không tươi|đồ cũ|đông lạnh lâu|hết hạn|ôxy hóa)\b",
            r"\b(khô cứng|nhão|dính răng|ngấy|quá ngọt|quá mặn|không tự nhiên)\b",
            r"\b(nguyên liệu kém|đồ đông lạnh|không tươi|để lâu|hết date)\b",
            r"\b(món).{0,10}?(không đáng giá|phí tiền|không nên thử|avoid)\b",
            r"\b(dở hơn|tệ hơn|kém hơn).{0,10}?(mong đợi|dự kiến|so với|chỗ khác)",
            r"\b(đồ ăn).{0,10}?(tồi tệ nhất|dở nhất|đáng thất vọng nhất)\b",
            r"\b(ăn vào muốn trả lại|không thể nuốt nổi|vứt đi|bỏ dở|gọi thêm vì đói nhưng không ăn được)\b",
            r"\b(snack tồi|đồ nhắm dở|food pairing tệ|worst food ever)\b",
            r"\b(tóc|móng tay|tàn thuốc|vật lạ).{0,10}(đồ ăn|món( ăn)?|thức ăn)\b"
        ]
...
def extract_drink_quality(text):
    drink_keywords = [
        ...
        r"\bmilkshake\b", r"\bjuice\b", r"\btea\b", r"\bcoffee\b", r"\bcafe\b", r"\bbeer\b", r"\bwine\b",
        r"\bchampagne\b", r"\bwhisky\b", r"\bvodka\b", r"\bliquor\b", r"\bbeverage\b", r"\bdrink\b",
        r"\bđá chanh\b", r"\btrà đá\b", r"\btrà đào\b", r"\bcốc\b", r"\bly\b", r"\bchai\b", r"\bnước\b"
    ]
    ...
    if existAspect:
        positive_patterns = [
            ...
            r"(đồ uống|nước uống|bia|trà|cà phê|rượu|cocktail).{0,15}?(ngon|thơm|chất lượng|đậm vị|tuyệt|hảo hạng|perfect|excellent)",
        ...
        negative_patterns = [
            r"\b(nhạt|lạt|nguội|đắng|chua|khó uống|dở|tanh|loãng|nhạt nhẽo|hôi|mùi lạ|kém|tệ|kinh khủng|thất vọng|không ngon|tệ|khó uống|nhạt|đắng|chua|hôi|loãng)\b",
            r"\b(quá ngọt|quá đắng|quá chua|quá nhạt|như hạch)\b",
...
def extract_drink_variety(text):
    ...
    if existAspect:
        positive_patterns = [
            ...
            r"\b(menu nước|thực đơn nước|drink menu|buffet nước|combo nước|set nước).{0,15}?(đa dạng|phong phú|nhiều lựa chọn|đầy đủ|complete|extensive|diverse)\b",
            r"\b(đủ các loại|từ A đến Z|tất cả các dòng|cập nhật mới nhất|đủ hãng|đủ vùng)\b",
            r"\b(đủ|đa dạng|nhiều) (các loại|dòng )?(coffee|cà phê|trà|rượu|bia|cocktail)\b",
            r"\b(barista|bartender|quầy bar).{0,10}?(giới thiệu nhiều|nhiều option|đa dạng)\b",
            r"\b(đa dạng nhất|phong phú nhất|nhiều lựa chọn nhất|hơn hẳn|vượt trội).{0,10}?(so với|mong đợi)",
            r"(uống gì cũng có|đủ món để uống)"
        ]
        negative_patterns = [
            r"\b(ít loại|ít lựa chọn|ít|nghèo nàn|hạn chế|kém đa dạng|thiếu|không (có )?nhiều|không đủ|hiếm|chỉ có vài|hạn hẹp)\b",
            ...
            r"\b(chỉ toàn|quanh đi quẩn lại|mãi không đổi|không cập nhật|chỉ có vài món)\b",
...
def extract_service_staff(text):
    ...
    staff_activities = [
        r"\b(cộc cằn|thái độ|phục vụ|nhiệt tình|thuyết minh|giới thiệu|hướng dẫn|tư vấn|giải đáp|đón tiếp|tiếp đón|quan tâm|dịch vụ|nói chuyện)\b",
    ...
    if existAspect:
        positive_patterns = [
            r"(?<!\bchưa\s)\b(tốt|tuyệt|ổn|đáng khen|hòa đồng|chuyên nghiệp|nhanh nhẹn|tươi cười|nhiệt tình|dễ thương|vui vẻ|hiếu khách|lịch sự|thân thiện|niềm nở|nhẹ nhàng|hết lòng|tận tâm|tận tình|chu đáo|quan tâm|tử tế)\b",
            r"\b(ấn tượng|hài lòng|thích|tốt).{0,10}?(với|về)?.{0,10}?(nhân viên|phục vụ|quản lý)\b",
            ...
            r"\b(professional|friendly|attentive|helpful|courteous|kind|excellent service|great staff)\b"
        ]
        negative_patterns = [
            r"\b(cục súc|tệ|dở|kém|đáng chê|thái độ|liếc|nói chuyện đùa giỡn|khó chịu|cộc cằn|gắt|khó ưa|thờ ơ|lạnh nhạt|vô duyên|khinh khỉnh|kiêu kỳ|hỗn|mất dạy|bất lịch sự|vô lễ)\b",
            r"\b(thái độ kém|phục vụ chậm|thiếu nhiệt tình|không quan tâm|không chú ý|thiếu chuyên nghiệp|không am hiểu)\b",
...
def extract_service_order(text):
    ...
    staff_orders = [
        r"\blên món\b", r"\bphục vụ\b", r"\border\b", r"\bgọi món\b", r"\bđặt món\b",
        r"\bchờ\b", r"\bđợi\b", r"\bmón lên\b", r"\bmón tới\b", r"\bthiếu món\b", r"\bsai món\b",
        r"\bphục vụ món\b", r"\bmang món\b", r"\bđem món\b", r"\bthời gian chờ\b",
        r"\bra món\b", r"\bthứ tự món\b", r"\btốc độ phục vụ\b", r"\btốc độ order\b", r"\bchờ đồ ăn\b",
        r"\bserving time\b", r"\bwaiting time\b", r"\border food\b", r"\bdish coming\b", r"\bfood served\b",
        r"\bmón ra\b", r"\bcấp món\b", r"\bthời gian lên món\b", r"\bthời gian phục vụ\b"
    ]
    ...
    if existAspect:
        positive_patterns = [
            r"(phục vụ|mang món|lên món|gọi món|ra món|đem đồ ăn).{0,15}?(nhanh|rất nhanh|liền|kịp lúc|tốt|đúng giờ|ngay lập tức|tức thì|nhanh chóng)",
            ...
            r"\b(fast service|quick serving|prompt service|efficient ordering|accurate order|correct dish)\b",
            r"\b(food came quickly|served immediately|order was perfect|no mistakes in order)\b"
        ]
        ...
        negative_patterns = [
            ...
            r"\b(đợi lâu).{0,10}?(nên (đồ|món|thức) ăn nguội (lạnh)?|mất ngon)\b",
            r"\b(slow service|long waiting|wrong order|missing dish|incorrect order|mixed up order|forgot my order)\b",
            r"\b(food took forever|served cold because late|dish came out wrong)\b"
        ]
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("./DatasetGGMaps/final_review_googlemap_comments_drinkplace.csv", index_col=0)
    df1 = pd.read_csv("./DatasetGGMaps/final_review_tripadvisor_comments_drinkplace.csv", index_col=0)
    
    df = append_aspects(df, processor)
    df1 = append_aspects(df1, processor)
    
    df_new = pd.concat([df, df1], ignore_index=True)
    df_new['category'] = df_new['category'].replace('cafe', 'drinkplace')
    df_new.to_csv('./annotated_labels/list_aspects_drinkplaces.csv')